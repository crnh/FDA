...
CITYSCAPES_PATH: Path = Path("/home/cornehaasjes/data/Cityscapes/train")
CITYSCAPES_LIST_PATH: Path = Path("/home/cornehaasjes/FDA/dataset/cityscapes_list")

GTA5_PATH: Path = Path("/home/cornehaasjes/data/GTAV/images")
GTA5_LIST_PATH: Path = Path("/home/cornehaasjes/FDA/dataset/gta5_list")
...
def get_list_sample(image_list: list[str], k=None):
    if k is None:
        ...
        if k > len(image_list):
            image_list *= len(image_list) // k + 1
        
        return sample(image_list, k=k)
