...
def main():
    ...
    for i in range(start_iter, args.num_steps):
        ...
        if (i+1) % args.print_freq == 0:
            ...
            imageio.imwrite(f"{args.tempdata}/src_img_{i}", src_img.cpu().numpy()[0].transpose((1, 2, 0))[:, :, ::-1].astype(np.uint8), format="png")
            imageio.imwrite(f"{args.tempdata}/trg_img_{i}", trg_img.cpu().numpy()[0].transpose((1, 2, 0))[:, :, ::-1].astype(np.uint8), format="png")
