...
def shape_low_freq_mutate( amp_src, amp_trg, beta, mask_type = 'circulair'):
    '''
    Exchange amplitude of the center of the target image (amp_trg) with that for the 
    source image (amp_src). Possible shapes 'square', 'circulair' or 'gauss'
    
    '''
    _, _, h, w = amp_src.size()
    beta_pixel = int((min(h,w)*beta)**2)# specificity the number of pixels in a square as depicted in the paper 
    

    if mask_type == 'square':
        b = (  np.floor(np.amin((h,w))*beta)  ).astype(int)     # get b
         
        amp_src[:,:,int(h/2)-int(b/2):int(h/2)+int(b/2),int(w/2)-int(b/2):int(w/2)+int(b/2)] = amp_trg[:,:,int(h/2)-int(b/2):int(h/2)+int(b/2),int(w/2)-int(b/2):int(w/2)+int(b/2)]
        
        return_amp = amp_src
       
    if mask_type == 'circulair':
        ...
        # apply mask
        return_amp = amp_trg * mask + amp_src * ~mask
    ...
    if mask_type == 'gauss':
        ...
        # fill gauss mask
        mask =  beta_pixel * norm_factor * (torch.exp(-(( (abs(x)-mu_x)**2 + (abs(y)-mu_y)**2)/ ( 2.0 * sigma**2 ) )))
        ...
        return_amp = (amp_trg * mask + amp_src * invers_mask) *4.5
        
    
    if mask_type != 'circulair' and mask_type != 'gauss' and mask_type != 'square':
        raise  Exception("mask_type not recognised, please use 'square', 'circulair' or 'gauss'")
        
          
    return return_amp


def shape_FDA_source_to_target(src_img, trg_img, beta=0.1, mask_type = 'circular'):
    ...
    # replace the low frequency amplitude part of source with that from target
    if mask_type == 'square': 
        amp_src_ = shape_low_freq_mutate( amp_src.clone(), amp_trg.clone(), beta, mask_type = 'square')
    if mask_type == 'circulair': 
        amp_src_ = shape_low_freq_mutate( amp_src.clone(), amp_trg.clone(), beta, mask_type = 'circulair')
    if mask_type == 'gauss': 
        amp_src_ = shape_low_freq_mutate( amp_src.clone(), amp_trg.clone(), beta, mask_type = 'gauss')
    if mask_type != 'circulair' and mask_type != 'gauss' and mask_type != 'square':
        raise  Exception("mask_type not recognised, please use 'square','circulair' or 'gauss'")
